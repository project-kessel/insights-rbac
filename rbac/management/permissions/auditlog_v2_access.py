...
# Granted by the `rbac:audit_log:read` V1 permission, which rbac-config maps to the
# `rbac_audit_log_view` V2 relation (`view` is the V2 read verb, as in `rbac_workspace_view`).
KESSEL_AUDIT_LOG_READ_RELATION = "rbac_audit_log_view"
...
class AuditLogV2KesselAccessPermission(permissions.BasePermission):
    ...
    Checks whether the principal holds the ``rbac_audit_log_view`` relation on the
