...
class FeatureFlags:
    ...
    def is_ocm_v2_enabled_global(self) -> bool:
        """Check the OCM V2 rollout flag without org context.

        Used by cross-organization endpoints (e.g. modified-tenants) where
        per-org evaluation is not feasible. Falls back to the OCM_V2_ENABLED
        environment variable when Unleash is unavailable.
        """
        return self.is_enabled(
            feature_name=self.TOGGLE_OCM_V2_ENABLED,
            fallback_function=lambda ignored_toggle_name, ignored_context: settings.OCM_V2_ENABLED,
