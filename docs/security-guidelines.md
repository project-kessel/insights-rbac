...
- gRPC channels use `grpc.insecure_channel()` in development/Clowder but `grpc.ssl_channel_credentials()` in production. The switch is in `create_client_channel()`, `create_client_channel_inventory()`, and `create_client_channel_inventory()` functions.
